            ...
            with tf.variable_scope("action_value"):
                if feature_extraction == "cnn":
                    ...
                    for layer_size in layers:
                        ...
                        action_out = tf_slim.fully_connected(action_out, num_outputs=layer_size, activation_fn=None)
                        if layer_norm:
                            #action_out = tf_layers.layer_norm(action_out, center=True, scale=True)
                            layer_norma = tf.keras.layers.LayerNormalization(axis=-1)
                            layer_norma(action_out)
                        action_out = act_fun(action_out)

                #action_scores = tf_layers.fully_connected(action_out, num_outputs=self.n_actions, activation_fn=None)
                action_scores = tf_slim.fully_connected(action_out, num_outputs=self.n_actions, activation_fn=None)
            ...
            if self.dueling:
                with tf.variable_scope("state_value"):
                    ...
                    for layer_size in layers:
                        ...
                        state_out = tf_slim.fully_connected(state_out, num_outputs=layer_size, activation_fn=None)
                        if layer_norm:
                            #state_out = tf_layers.layer_norm(state_out, center=True, scale=True)
                            layer_norma = tf.keras.layers.LayerNormalization(axis=-1)
                            layer_norma(state_out)
